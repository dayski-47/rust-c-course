...
## SQLx in CI: The Offline Mode Problem

`sqlx::query!` needs a live database at compile time to verify your SQL. In CI
(GitHub Actions, etc.) you usually don't have one. This breaks your build.
...
With `SQLX_OFFLINE=true`, sqlx reads from `.sqlx/` instead of connecting to a
database. Your CI build works without any database setup.

**When to regenerate:** Any time you add, change, or remove a `sqlx::query!` call,
run `cargo sqlx prepare --workspace` again and commit the updated `.sqlx/` directory.
If you forget, CI will reject the build with "query in `.sqlx` is outdated."

**Add this check to CI** to catch stale metadata automatically:

```yaml
- name: Check sqlx offline data is current
  env:
