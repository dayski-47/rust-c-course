...
In Axum, extract it alongside `WebSocketUpgrade`:
...
pub async fn ws_queue_handler(
    ...
    // Only reach here if the token is valid
    ws.on_upgrade(move |socket| async move {
        handle_queue_socket(socket, claims, state).await;
    })
}
```

**Approach 2: Send token as first message** (for non-browser clients or when
query params feel wrong):
...
pub async fn handle_queue_socket(
    ...
    let claims = match socket.recv().await {
        Some(Ok(Message::Text(text))) => {
            let msg: AuthMessage = serde_json::from_str(&text)
                .expect("Expected JSON auth message");
            match verify_access_token(&msg.token, &state.jwt_secret) {
                Ok(c) => c,
                Err(_) => {
    ...
    handle_authenticated_session(socket, claims, state).await;
}
```

The query parameter approach is simpler and works for most use cases. The
first-message approach avoids exposing tokens in server logs (query params often
appear in access logs).

**Important:** Even with query parameter auth, the `ws.on_upgrade()` must not be
called if the token is invalid. Returning before `on_upgrade` closes the connection
with a non-101 status code, which the client sees as a connection failure.
