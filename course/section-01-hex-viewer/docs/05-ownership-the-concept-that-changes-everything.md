...
| C++ | Rust | Key difference |
|-----|------|----------------|
| `std::unique_ptr<T>` | `Box<T>` | Use-after-move is a compile error in Rust |
| `std::shared_ptr<T>` | `Arc<T>` (thread-safe) | Reference counting is explicit |
| `std::shared_ptr<T>` (single-thread) | `Rc<T>` | Compiler prevents using Rc across threads |
