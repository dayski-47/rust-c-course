...
## Object Safety: The Exact Rules

A trait is object-safe (usable as `dyn Trait`) if every method satisfies ALL of these:
1. The method doesn't use `Self` as a parameter or return type (except `Box<Self>`)
2. The method doesn't have generic type parameters
3. The method has a receiver (`&self`, `&mut self`, or `Box<Self>`)
4. No associated functions (no `Self::new()` without a receiver)

```rust
trait ObjectSafe {
    fn process(&self, data: &[u8]) -> Vec<u8>;  // ✅ fine
    fn name(&self) -> &str;                      // ✅ fine
}

trait NotObjectSafe {
    fn clone(&self) -> Self;          // ❌ returns Self
    fn compare<T: PartialEq>(&self, other: T); // ❌ generic method
    fn new() -> Self;                 // ❌ no receiver
}
```

When a trait is NOT object safe, the compiler tells you: "the trait cannot be made into an object." The fix is usually: 1) remove the problematic method, 2) add it to a separate non-object-safe trait, or 3) use `where Self: Sized` to exclude it from the vtable:
...
pub enum JobError {
    ...
    Io(#[from] std::io::Error),  // auto-generates From<io::Error> for JobError
}
```

The `#[from]` attribute is key: it generates `impl From<io::Error> for JobError` automatically. This means inside your job implementation, any `io::Error` can be converted with `?`:

```rust
fn execute(&self) -> Result<(), JobError> {
    let file = File::open(&self.path)?; // io::Error → JobError::Io via #[from]
    // ...
    Ok(())
}
```
...
fn main() -> Result<()> {
    let pool = TaskPool::connect(&config.db_url)
        .context("failed to connect to task database")?;
    
    pool.run().await
        .context("task pool exited with error")?;
...
When an error propagates up through multiple layers, each layer adds context. This is how you get errors that read like "failed to start worker: failed to connect to database: connection refused on port 5432":

```rust
use anyhow::Context;

fn start_worker(config: &Config) -> anyhow::Result<()> {
    let pool = connect_to_db(&config.db_url)
        .context("failed to connect to database")?;  // adds outer context
...
fn connect_to_db(url: &str) -> anyhow::Result<DbPool> {
    DbPool::connect(url)
        .context(format!("connection refused on {url}"))?;  // adds inner context
    // ...
}

// Error chain when printed:
// failed to start worker
// caused by: failed to connect to database  
// caused by: connection refused on postgres://localhost/taskdb
```

The context is accumulated as a chain. Each `.context()` call wraps the existing error with a new message. When you print the error with `{:#}` (alternate format), the entire chain is shown.
