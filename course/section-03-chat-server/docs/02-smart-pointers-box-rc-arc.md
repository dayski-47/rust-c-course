...
// let r2 = data.borrow_mut();  // ❌ panics: already borrowed as immutable
```
...
| Situation | Use |
|-----------|-----|
| Multithreaded shared mutation | `Arc<Mutex<T>>` |
| Single-threaded, complex data, dynamic borrow pattern | `RefCell<T>` |
| Single-threaded, Copy type, simple get/set | `Cell<T>` |
| Single-threaded, many readers or one writer | `RefCell<T>` with `.borrow()`/`.borrow_mut()` |
